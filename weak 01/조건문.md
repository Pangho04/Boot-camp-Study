...
if (조건A) {
    ...
    수행문 // 조건A가 거짓이며 조건B가 참일 때 실행
} else {
    수행문 // 조건A, B가 모두 거짓일 때 실행
}
```

### 조건
* 조건이 truthy 값일 경우 수행문이 실행된다.
* truthy 값: 불리언 타입으로 형변환 시 true로 간주되는 값, falsy 값을 제외한 모든 값을 의미한다.
* falsy 값: `false`, `0`, `-0`, `0n`, 빈 문자열 `""`, `null`, `undefined`, `NaN`, and `document.all`
...
//"a is not 1"
```
* `if...else`문은 `if`와 `else if`을 중첩하여 사용할 수 있다.   
* 혼동과 오류를 피하기 위해 `블록 문{...}`을 사용한다.

<br>

```js
function menu(cuisine) {
  if (cuisine === "한식") {
    return "김치찌개";
  } else if (cuisine === "중식") {
    return "짜장면";
...
console.log(menu("아무거나")); // "햄버거"
```

* `if...else if...else` 문을 통해 다양한 조건을 추가할 수 있다.

<br>

# Ternary operator
* 삼항 연산자는 `물음표(?)` 뒤에 세가지 피연산자를 받는 JavaScript 유일한 연산자 이다.  
* 삼항 연산자의 피연산자: `조건문`, `실행식(truthy)`, `실행식(falsy)`
...
- 조건문으로 사용될 표현식
### 실행문(truthy)
- 조건이 `true`로 평가될 시 실행될 구문.
### 실행문(falsy)
- `falsy 값`으로 평가될 시 실행될 구문(`false`, `0`, `-0`, `0n`, `""`, `null`, `undefined`, `NaN`, and `document.all`)
## 예문
```js
const age = 26;
...
* `break`문이 없다면 그 아래의 케이스표현식을 평가하지 않고 수행문을 실행한다. (<span style='color:gray; background-color:#fdf84fe0;'>fall-through</span>)
...
## 주의점
> 💡 `case`문과 `default`문은 스스로 스코프를 만들지 않는다.   
별도의 스코프를 지정하고 싶을 경우 콜론 뒤에 `{}`로 직접 `블록 스코프`를 만들어 주어야 한다.
```js
const action = "say_hello";

switch (action) {
  case "say_hello": { // block
    const message = "hello";
    console.log(message);
    break;
  }
...
```
* 위 예문에서 `case` 문에 중괄호가 없을 경우 `const` 가 재선언되기 때문에 `SyntaxError` 가 발생한다.   
_(SyntaxError: Identifier 'message' has already been declared)_
## 예문
```js
...
* case절 표현식은 필요한 경우에만 실행되고, 이미 동일한 표현식을 찾은 경우엔 실행하지 않는다.
