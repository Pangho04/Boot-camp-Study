...
export default function characterFrequency(string) {
  ...
  do {
    if (words[i] === words[i-1]) {
      i++;
      continue;}
    let count = words.filter((word) => word ===words[i])
    result.push([words[i], count.length])
    i++;
  } while (i < words.length )
  result.sort(function(a,b){ if (a[1] === b[1]) {
    return 0;
    } return b[1] - a[1]
  });
   return result;
